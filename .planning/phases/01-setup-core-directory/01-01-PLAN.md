...
<task type="auto">
  ...
  <action>
    ...
    - description: text
    - role: varchar(100) not null (category - implements DIR-03 taxonomy)
    ...
    **DIR-03 Category Taxonomy Implementation:**
    Add CHECK constraint or enum for role column to enforce the 10 categories:
    'Builder', 'Craftsperson', 'Tradesperson', 'Developer', 'Investor', 'Advocate', 'Architect', 'Government', 'Nonprofit', 'Educator'

    This ensures database-level validation that listings are organized by the approved category taxonomy.
  ...
  <verify>
    ...
    Check drizzle/migrations/ for SQL file containing CREATE TABLE with PostGIS geometry column and role constraint
  </verify>
  <done>
    Database schema defined with PostGIS geometry column, full-text search support, proper indexes, and DIR-03 category taxonomy enforcement
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    - Export Category type with DIR-03 taxonomy: 'Builder' | 'Craftsperson' | 'Tradesperson' | 'Developer' | 'Investor' | 'Advocate' | 'Architect' | 'Government' | 'Nonprofit' | 'Educator'
    ...
    - listingSchema: Validates listing data structure, includes enum validation for category field matching the 10 DIR-03 categories
  ...
  <verify>
    ...
    Check that src/types/index.ts exports Listing and Category types with all 10 categories
  </verify>
  <done>
    TypeScript types generated from schema with DIR-03 category taxonomy enforced, Zod validation schemas created, no build errors
  </done>
</task>
...
2. Database schema file exists with PostGIS geometry column and DIR-03 category constraints
...
- DIR-03 category taxonomy enforced at database and TypeScript levels
